...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    // Create database
    let mut connection = PgConnection::connect_with(&config.without_db().expose_secret())
        .await
    ...
    // Migrate database
    let connection_pool = PgPool::connect_with(&config.with_db().expose_secret())
        .await
        .expect("Failed to connect to Postgres.");
...
async fn subscribe_returns_a_200_for_valid_form_data() {
    ...
    // getting the connection from spawn_app()
